...
void pcaInit() {
  ...
  for(uint8_t i=0; i<NUM_PCA; i++) {
    ...
    if (checkI2CConnection(PCA_ADDRESS1, &Serial)) {
      ...
      if(i==0) getAndAttach16ServosToPCA9685Expander(PCA_ADDRESS1);
      if(i==1) getAndAttach16ServosToPCA9685Expander(PCA_ADDRESS2);
    }
  }
  ...
  setEasingTypeForAllServos(EASE_CUBIC_IN_OUT); //
    ...
    writeAllServos(90);
  #else
    for(U8 ch=0; ch<num_pca_servo; ch++) {
      if( !pcaavail[ch/16] ) continue;
      U8 a1 = NODECONFIG.read( EEADDR(pcaservo[ch].angle1) );
      U8 a2 = NODECONFIG.read( EEADDR(pcaservo[ch].angle2) );
      pcawrite(ch, (a1+a2)/2);
    }
  #endif
...
//
  ...
  if( index<NUM_PCA_SERVO_EVENT) {
    ...
    uint8_t mdpt = (t1+t2)/2;    // cal the midpoint
    dP(" posn1="); dP(t1); dP(" posn2="); dP(t2);
    if(e==0) {
