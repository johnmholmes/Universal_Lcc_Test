...
// ===== Enter User definitions below =====
  CDIheader R"(
    ...
    <group replication=')" N(NUM_PCA_PORTS) R"('>
      ...
      <hints><visibility hideable='yes' hidden='yes' ></visibility></hints>
      <repname>First PCA Channels 0-7</repname>
      <repname>First Channels 8-15</repname>
      <repname>Second Channels 0-7</repname>
      <repname>Second Channels 8-15</repname>
      <group replication=')" N(8) R"('>
        <name>Servos</name>
        <repname>0 or 8</repname>
        <repname>1 or 9</repname>
        <repname>2 or 10</repname>
        <repname>3 or 11</repname>
        <repname>4 or 12</repname>
        <repname>5 or 13</repname>
        <repname>6 or 14</repname>
...
//   Memory structure of EEPROM, must match CDI above
    typedef struct {
      ...
      // ===== Enter User definitions above =====
      uint8_t curpos[ NUM_NAT_SERVO];  // save current positions of servos
      // items below will be included in the EEPROM, but are not part of the CDI
    } MemStruct;                 // type definition

uint8_t curpos[ NUM_NAT_SERVO]; 
...
Servo servo[2];
uint8_t  natservodelay;
//uint8_t servopin[ NUM_NAT_SERVO] = {25,33};  // CHOOSE PINS FOR SERVOS
uint8_t servopin[ NUM_NAT_SERVO] = { SERVOPINS };  // CHOOSE PINS FOR SERVOS
uint8_t servoActual[ NUM_NAT_SERVO];
uint8_t servoTarget[ NUM_NAT_SERVO];
...
{ 
  ...
  NODECONFIG.update(EEADDR( natservodelay), 50);
  NODECONFIG.update(EEADDR(saveperiod), 50);
  for(uint8_t i = 0; i <  NUM_NAT_SERVO; i++) {
    NODECONFIG.put(EEADDR(servos[i].desc), ESTRING(""));
    for(int p=0; p< NUM_NAT_POS; p++) {
      ///NODECONFIG.update(EEADDR(servos[i].pos[p].angle), 60+p*30);
      NODECONFIG.update(EEADDR(servos[i].pos[p].angle), 90);
      NODECONFIG.update(EEADDR(curpos[i]), 1);
    }
  }
  dP("\n NUM_IO"); dP(NUM_IO);
...
uint8_t userState(uint16_t index) {
  //dP("\n userState "); dP((uint16_t) index);
    ...
    index -=  NUM_NAT_SERVO_EVENT;
    if( index<NUM_IO_EVENT ) {
      int ch = (index -  NUM_NAT_SERVO* NUM_NAT_POS)/2;
...
// PCA9685 ROUTINES
uint8_t target[NUM_PCA_SERVO];
uint16_t pcabase =  NUM_NAT_SERVO_EVENT + NUM_IO_EVENT;
...
//
  ...
  if(index< NUM_NAT_SERVO_EVENT) {
    ...
    //dP(" servoTarget="); dP(servoTarget[outputIndex]); dP(" curpos="); dP(curpos[outputIndex]); 
    return;
  }
  // All IO, both native and MCP23017
  index -=  NUM_NAT_SERVO_EVENT; // adjust index
...
void natServoProcess() {
  static long natservolast = 0;
  if( (millis()-natservolast) < natservodelay ) return;
  natservolast = millis();
  static long lastmove = 0;
  for(int i=0; i< NUM_NAT_SERVO; i++) {
    if(servoTarget[i] == servoActual[i] ) continue;
    //dP("\n natServoProcess natservodelay="); dP(natservodelay); dP("\n i="); dP(i); dP(" servoActual="); dP(servoActual[i]); dP("\n servoTarget="); dP(servoTarget[i]);
    ///if(servoTarget[i] > servoActual[i] ) servoActual[i]++;
    ///else if(servoTarget[i] < servoActual[i] ) servoActual[i]--;
    uint8_t dd = 200/natservodelay;
    //if( natservodelay<20 ) {
  ...
  if( lastmove && (millis()-lastmove)>4000) {
    for(int i=0; i< NUM_NAT_SERVO; i++) servo[i].detach();
    lastmove = 0;
    //dP("\n detach()");
  }
  ...
  if(saveperiod && posdirty && (millis()-lastsave) > saveperiod ) {
    ...
    for(int i=0; i< NUM_NAT_SERVO; i++) NODECONFIG.update( EEADDR(curpos[i]), curpos[i]);
    EEPROMcommit;
...
void produceFromInputs() {
    ...
    // with pce.produce(i);
    const uint8_t base =  NUM_NAT_SERVO* NUM_NAT_POS;
    static uint8_t c = 0;
...
void userConfigWritten(uint32_t address, uint16_t length, uint16_t func)
{
  ...
  setupIOPins();
  if( address == EEADDR( natservodelay))  { dP("\n GET natservodelay"); natservodelay = NODECONFIG.read( EEADDR(  natservodelay ) ); }
  pcaServoSet(address);
}
...
void natServoStartUp() {
  dP("\n natServoStartUp");
   natservodelay = NODECONFIG.read( EEADDR(  natservodelay ) );
  for(int i=0; i< NUM_NAT_SERVO; i++) {
...
// Redo Servo adjustments from EEPROM
void natServoSet() {
  for(int i=0; i< NUM_NAT_SERVO; i++) {
...
void appProcess() {
  uint8_t base =  NUM_NAT_SERVO *  NUM_NAT_POS;
  unsigned long now = millis();
...
void setup()
{
    ...
    dP("\n 2Servo8IO2MCP2PCA");
    dP("\n num servos = "); dP( NUM_NAT_SERVO);
    dP("\n num servo pos = "); dP( NUM_NAT_POS);
    ...
    dP("\n num native servo events = "); dP( NUM_NAT_SERVO_EVENT);
  ...
  dP("\n NUM_EVENT="); dP(NUM_EVENT);
      ...
      dP("\n"); dP( 8+EEADDR( natservodelay ),HEX ); dP(" natservodelay="); dP( NODECONFIG.read( EEADDR(natservodelay) ), HEX );
      dP("\n"); dP( 8+EEADDR( saveperiod ),HEX );    dP(" saveperiod=");    dP( NODECONFIG.read( EEADDR(saveperiod) ), HEX );
      dP("\nNative Servos:");
      for(int s=0; s<2; s++) 
        for(int p=0; p<3; p++) 
          {dP("\n"); dP( 8+EEADDR( servos[s].pos[p].angle ),HEX ); dP(" servos[s].pos[p].angle="); dP( NODECONFIG.read( EEADDR(servos[s].pos[p].angle) ), HEX );}
      dP("\nNative and MCP IO:");
      for(int i=0; i<NUM_IO; i++) {
        dP("\n"); dP(i); 
      ...
      for(int i=0; i<NUM_PCA_SERVO; i++) {
        dP("\n"); dP(i); 
        dP("\n   "); dP( 8+EEADDR( pcaservo[i].speed ),HEX );  dP(" speed=");   dP( NODECONFIG.read( EEADDR( pcaservo[i].speed ) ), HEX );
        dP("\n   "); dP( 8+EEADDR( pcaservo[i].angle1 ),HEX ); dP(" angle1 ="); dP( NODECONFIG.read( EEADDR( pcaservo[i].angle1 ) ), HEX );
        dP("\n   "); dP( 8+EEADDR( pcaservo[i].angle2 ),HEX ); dP(" angle2 ="); dP( NODECONFIG.read( EEADDR( pcaservo[i].angle2 ) ), HEX );
      }
      dP("\n"); dP( EEADDR( pcaStartupPosition ) ); dP(" pcaStartupPosition="); dP( NODECONFIG.read( EEADDR( pcaStartupPosition ) ) );
  #endif
  ...
  if(c=='S') { // native servo
      ...
      switch (cc) {
        case 's': { // speed
            ...
            NODECONFIG.write( EEADDR( natservodelay ), v );
            EEPROM.commit();
            natservodelay = v;
            dP("\n Setting servodelay to "); dP(v);
          }
          break;
        case 'm': { // speed
            ...
            if(p>2) break;
            dP("\n ==>Move servo "); dP(s); dP(" to position "); dP(p); 
            pceCallback( s*3+p );
          }
          break;
        case 'a': { // speed
            uint8_t s = Serial.parseInt(); // servo#
            if(s>1) break;
            uint8_t a1 = Serial.parseInt(); // angles
            uint8_t a2 = Serial.parseInt(); //
            uint8_t a3 = Serial.parseInt(); // 
            NODECONFIG.write( EEADDR( servos[s].pos[0].angle ), a1 );
            NODECONFIG.write( EEADDR( servos[s].pos[1].angle ), a2 );
            NODECONFIG.write( EEADDR( servos[s].pos[2].angle ), a3 );
            EEPROM.commit();
            dP("\n Setting servo "); dP(s); dP("'s angles to "); dP(a1); dP(","); dP(a2); dP(","); dP(a3);
          }
          break;        
        case 'l': { // speed
            dP("\n\nNative Servos:");
            for(int s=0; s<2; s++) {
              dP("\n "); dP(s);
              for(int p=0; p<3; p++) {
                dP(" pos="); dP(p); dP("/angle="); dP( NODECONFIG.read( EEADDR( servos[s].pos[p].angle ) ) );
              }
            }
          }
          break;  
      ...
      switch (cc) {
        case 's': {
            ...
            uint8_t p = Serial.parseInt();
            NODECONFIG.write( EEADDR( io[i].type), t );
            NODECONFIG.write( EEADDR( io[i].duration), d );
            NODECONFIG.write( EEADDR( io[i].period), p );
            EEPROM.commit();
        ...
        case 'o': {
            ...
            if(i>=NUM_IO) break;
            dP("\n ==> Turn IO "); dP(i); dP(" ON");
            pceCallback( NUM_NAT_SERVO_EVENT + i*2 );
          }
          break;
        case 'f': {
            uint8_t i = Serial.parseInt(); // io#
            if(i>=NUM_IO) break;
            pceCallback( NUM_NAT_SERVO_EVENT + i*2+1 );
          }
          break;
      }
  } else if(c=='P') {  // PCA
      ...
      switch (cc) {
        case 'a': { // set angles
            ...
            NODECONFIG.write( EEADDR( pcaservo[s].angle1), angle1 );
            NODECONFIG.write( EEADDR( pcaservo[s].angle2), angle2 );
            EEPROM.commit();
        ...
        case 'm': { // move servo position
            ...
            if(p<=0) pceCallback( NUM_NAT_SERVO_EVENT+ NUM_IO_EVENT + s*4 );
            if(p>=1) pceCallback( NUM_NAT_SERVO_EVENT+ NUM_IO_EVENT + s*4+1 );
          }
          break;
        case 'v': { // speed
            uint8_t s = Serial.parseInt();
            if(s>=NUM_PCA_SERVO) break;
            uint8_t v = Serial.parseInt();
            NODECONFIG.write( EEADDR( pcaservo[s].speed), v);
            EEPROM.commit();
        ...
        case 'l': // list
          dP("\n\nPCA Servos");
          for(int s=0; s<NUM_PCA_SERVO; s++) {
            if( !pcaavail[s/16] ) continue;
            dP("\n"); dP(s); 
            dP(" angle1="); dP(NODECONFIG.read( EEADDR( pcaservo[s].angle1))); 
            dP(" angle2="); dP(NODECONFIG.read( EEADDR( pcaservo[s].angle2))); 
          }
          break;
